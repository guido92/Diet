...
export default async function PlanPage() {
    ...
    // If no session, show Login (Should ideally be a redirect or handled by middleware, but keeping logic)
    if (!session) {
        return (
            <div style={{
                display: 'flex',
    ...
    return (
        <div style={{ padding: '1rem', maxWidth: '600px', margin: '0 auto', paddingBottom: '100px' }}>
            ...
            {/* HEADER: Dynamic Greeting - Keeping for context in Plan View too, but maybe simplified? */}
            <header className="flex-between" style={{ marginBottom: '1.5rem' }}>
                <div>
                    ...
                    <h1 className="title" style={{ margin: 0, fontSize: '1.8rem' }}>Piano di Oggi</h1>
                </div>

            </header>

            {/* REMOVED: Water Tracker and Next Meal Widget (Moving to Home) */}

            {/* MEAL PREP ALERT (Keeping here as well for reference) */}
            {shouldCookDouble && (
            ...
            {/* TODAY'S FULL PLAN */}
            <h2 className="title" style={{ marginTop: '30px', marginBottom: '15px', fontSize: '1.3rem' }}>Programma Completo</h2>
            ...
            <div style={{ marginTop: '40px', display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '10px' }}>
                <Link href="/planner" className="btn" style={{ background: '#334155', minWidth: '120px', justifyContent: 'center' }}>📅 Settimana</Link>
            </div>
